...
namespace BlazorApp.Server.Services
{
    public class AdminService : grpcAdminService.grpcAdminServiceBase
    {
        ...
        public override async Task<GrpcLogin_Response> GrpcLogin(Admin.Services.GrpcLogin_Request request, ServerCallContext context)
        {
            ...
            try
            {
                ...
                if (findRecord != null)
                {
                    ...
                    response.AccessToken = AccessTokenService.Generate_AccessToken(findRecord.UserName, findRecord.RoleID);
                }
                else
                {
        ...
        // CheckAccessToken
        public override async Task<Admin.Services.String_Response> CheckAccessToken(CheckAccessToken_Request request, ServerCallContext context)
        {
            ...
            {
                //Check
                if (!AccessTokenService.Check_AccessToken(request.UserName, request.RoleID, request.AccessToken))
                {
                    response.ReturnCode = GrpcReturnCode.Error_201;
                }

                //Generate new access token
                response.StringValue = AccessTokenService.Generate_AccessToken(request.UserName, request.RoleID);
            }
            catch
            {
                response.ReturnCode = GrpcReturnCode.Error_201;
